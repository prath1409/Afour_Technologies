...
export const requestBook = async (bookName: string, numberParams: number, reqUser: User): Promise<Books> => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if(!bookName)
                return resolve({status: RES_STATUS.BAD_REQUEST, message: 'Book name required'});
            if(numberParams != 1)
                return resolve({status: RES_STATUS.BAD_REQUEST, message: 'Only one parameters required'});
            ...
            if(!book.rowCount)
                return resolve({status: RES_STATUS.BAD_REQUEST, message: 'Book name invalid'});
...
export const returnBook =async (bookName: string, reqUser: User): Promise<Books> => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if(!book.rowCount)
                return resolve({status: RES_STATUS.BAD_REQUEST, message: 'Book Id invalid'});
            const bookId: string = book.rows[0].id
